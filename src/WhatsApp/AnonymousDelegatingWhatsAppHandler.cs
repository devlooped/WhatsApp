...
class AnonymousDelegatingWhatsAppHandler(
    ...
    Func<IEnumerable<IMessage>, IWhatsAppHandler, CancellationToken, IAsyncEnumerable<Response>> handlerFunc) : DelegatingWhatsAppHandler(innerHandler)
{
    /// <summary>The delegate to use as the implementation of <see cref="Handle"/>.</summary>
    readonly Func<IEnumerable<IMessage>, IWhatsAppHandler, CancellationToken, IAsyncEnumerable<Response>> handlerFunc = Throw.IfNull(handlerFunc);
