...
/// <summary>
/// Default implementation of the <see cref="IWhatsAppFlowsClient"/>.
/// </summary>
...
public class WhatsAppFlowsClient(IHttpClientFactory httpFactory, IOptions<MetaOptions> options, ILogger<WhatsAppClient> logger) : IWhatsAppFlowsClient
{
    ...
    /// <inheritdoc />
    public HttpClient CreateHttp(string accountId)
    {
        if (!options.Accounts.TryGetValue(accountId, out var token))
            throw new ArgumentException($"The account '{accountId}' is not registered in the options.", nameof(accountId));
