...
- `Id`, `UserId`, `ServiceId`, `Timestamp`, `Context`, `Type`
...
- Holds `Service` (id + phone number), `User` (name + `Id` + optional `Number`), `Timestamp`. `User.Id` is either a phone number or a Business-Scoped User ID (BSUID, shape `CC.alphanumeric`). `User.Number` is `null` when the user opted for username-only privacy. `User.IsBSUID` reflects the detection.
...
| `FlowToken` | Encodes `ServiceId` + `UserId` + `Flow` name in the flow token |
...
- **`changelog.md`** — auto-generated by the release workflow from git history and tags. Never edit or stage this file manually; any local changes to it should be discarded before committing.
